...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		\paragraph{Conclusioni}
		Il gruppo è riuscito a mantenere invariato il numero di ore che erano state preventivate per l'incremento, tuttavia è stato necessario effettuare un minimo cambiamento nella suddivisione oraria per ruolo, il quale ha coinvolto il ruolo di progettista e di programmatore. Infatti il design delle \glock{API} rimanenti ha richiesto un'ora in più di quanto previsto, ma la loro buona progettazione ne ha consentito un'implementazione più rapida, risparmiando un'ora di codifica.
		\newline
		Questo cambiamento, sebbene non abbia influito sul numero totale di ore di lavoro impiegate, ha comportato una spesa di € 7,00 maggiore rispetto a quanto preventivato
		
		\paragraph{Preventivo a finire}
		Lo sviluppo procede in linea con quanto pianificato, nonostante il piccolo imprevisto verificatosi durante la progettazione delle API. Infatti il gruppo è stato pronto a reagire, dedicando un'ora in più al design delle funzionalità e, grazie all'esperienza pregressa, quel tempo è stato sfruttato al meglio, permettendone il recupero durante la codifica.
		\newline
		Sebbene tutto ciò abbia comportato un superamento di € 7,00 del costo totale preventivato, sono state ugualmente realizzate tutte le funzionalità previste per l'incremento e, considerato il risparmio complessivo di € 15,00 alla data attuale, non si ritiene necessario apportare modifiche al preventivo definito in \S5. Il gruppo confida, infatti, di concludere lo sviluppo con il prossimo incremento, rispettando la pianificazione.
		...
		\newline
		Tuttavia, per completare la codifica della funzionalità di invio dei comandi tramite il \glock{bot Telegram}, sono state necessarie due ore in più del ruolo di programmatore; questo è stato necessario per permettere l'inserimento di ulteriori istanze \glock{Axios} per la gestione delle richieste richieste inviate dagli utenti.
		\newline
		In seguito a questo cambiamento, per mantenere il costo totale invariato rispetto al preventivo, è stato necessario rinunciare a due ore del ruolo di verificatore.
		...
		L'esperienza si è conclusa, il gruppo è più che entusiasta di quanto prodotto e del modo in cui sono stati raggiunti tutti gli obbiettivi prefissati. Nonostante le difficoltà riscontrate ad inizio progetto, causate soprattutto dalla poca esperienza in merito, siamo riusciti a rispettare tutte le scadenze stabilite. 
		L'ottima collaborazione tra membri è stata essenziale, e ha fatto si che le soluzioni ai problemi affrontati durante le varie fasi, siano state individuate in modo efficiente ed efficace.
		
		DA CONTINUARE
	
		Il costo totale per lo sviluppo del progetto rimane in linea con quanto preventivato, ammontando a 13.786,00 €.
