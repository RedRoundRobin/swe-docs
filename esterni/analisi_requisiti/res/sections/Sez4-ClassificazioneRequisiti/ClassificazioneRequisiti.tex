...
In questa parte, vengono riportati i requisiti del progetto, classificati per tipologia . Ciascun requisito possiede un codice identificativo, il cui formalismo viene riportato all'interno del documento \dext{Norme di Progetto v1.0.0}.
	...
	\begin{center}
		...
		\req{A}{F}{60} 		& Il sistema deve disattivare automaticamente le pagine \textit{View} attive per un utente, qualora l'utente venga disattivato & UC \autism
		\req{A}{F}{61} 		& Il sistema deve disattivare automaticamente le pagine \textit{View} attive per un utente, qualora l'utente venga disattivato & UC \autism
		\req{A}{F}{62} 		& Il sistema non deve permettere l'accesso a utenti non amministratori che non fanno parte di un ente & UC \autism
		\req{A}{F}{63} 		& Il sistema non deve permettere l'accesso a utenti disattivati & UC \autism
		\req{A}{F}{64} 		& Il sistema non deve permettere notifica degli alert agli utenti disattivati & UC \autism
		\req{A}{F}{65} 		& Il sistema deve permettere notifica degli alert in base alle preferenze indicate nelle impostazioni di un utente & UC \autism
		\req{A}{F}{66}  	& I dati usati nel sistema devono essere salvati all'interno di una base di dati & UC \autism
		\sreq{A}{F}{66.1} 	& I dati ricevuti dai gateway e usati per la creazione dei grafici vanno salvati all'interno di un time-series database (non relazionale). & UC \autism
		\sreq{A}{F}{66.2} 	& I dati utilizzati per gli account utenti devono essere salvati in un database relazionale. & UC \autism
		% API
		\req{A}{F}{67} 		& Le applicazioni che vogliono utilizzare il sistema devono interfacciarsi tramite delle \glock{API} che si mettono in comunicazione con \glock{Kafka} & UC \autism
		\sreq{A}{F}{67.1} 	& La web app deve interfacciarsi con le \glock{API} & UC \autism
		\sreq{A}{F}{67.2} 	& Il bot di \glock{Telegram} deve interfacciarsi con le \glock{API} & UC \autism
		\hline
		% gateway
		\req{A}{F}{68} 		& Le configurazioni dei \glock{Gateway} devono essere inviate dagli amministratori attraverso le \glock{API} & UC \autism
		\sreq{A}{F}{68.1} 	& Le configurazioni dei \glock{Gateway} devono poter essere sovrascritte & UC \autism
		\req{A}{F}{69} 		& Le \glock{API} devono interfacciarsi necessariamente con \glock{Kafka} per la ricezione e invio di dati & UC \autism
		\req{A}{F}{70} 		& La base di dati si deve interfacciare con \glock{Kafka} per la scrittura e lettura dei dati & UC \autism
		\req{A}{F}{71} 		& I \glock{Gateway} comunicano con \glock{Kafka} tramite \glock{topic} con il modello \glock{Producer-Consumer} & UC \autism
		\req{A}{F}{72} 		& I dispositivi comunicano con i \glock{Gateway} i dati da inviare & UC \autism
