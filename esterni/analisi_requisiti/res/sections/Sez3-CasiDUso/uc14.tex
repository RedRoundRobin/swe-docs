\subsection{UC14 - Telegram - Autenticazione}
	...
	\begin{itemize}
		...
		\item \textbf{Descrizione}: L'utente tenta di autenticarsi con il bot \glock{Telegram}, per farlo entra all'interno dell'applicazione e inizia la chat con il bot. Attraverso \glock{Telegram}, viene segnalato al bot il nome utente e il bot verifica se questo è censito all'interno del sistema. Se viene riconosciuto, viene registrata l'apertura e l'autenticazione del canale di comunicazione tra il bot e l'utente, diventando quest'ultimo autenticato. 
	...
	\begin{itemize}
		\item \textbf{Attori Primari}: Utente non autenticato.
		\item \textbf{Descrizione}: L'utente invia una richiesta al bot di volersi autenticare per poter aprire il canale di comunicazione.
		\item \textbf{Precondizione}: L'utente è nell'applicazione di \glock{Telegram} e non ha una chat autenticata con il bot.
		\item \textbf{Postcondizione}: L'utente invia il comando per richiedere l'autenticazione.
		\item \textbf{Scenario Principale}:
		\begin{enumerate}
			\item L'utente seleziona nell'applicazione di \glock{Telegram} il bot del sistema.
			\item L'utente tenta di aprire una conversazione con il bot.
			\item Per attivare la conversazione con il bot ed autenticarsi, l'utente invia il comando predefinito.
		\end{enumerate}
	...
	\begin{itemize}
		...
		\item \textbf{Precondizione}: L'utente, tramite \glock{Telegram}, ha inviato al bot una richiesta di avvio conversazione con il comando predefinito.
		\item \textbf{Postcondizione}: L'utente effettua il riconoscimento con il sistema e viene autenticato.
		\item \textbf{Scenario Principale}:
		\begin{enumerate}
			\item Il \glock{Telegram} bot riceve in input il comando che viene inoltrato al sistema, insieme alle informazioni relative alla chat e all'autore del messaggio.
			\item Viene mostrato un messaggio di benvenuto dal bot, attraverso \glock{Telegram}, che conferma le credenziali dell'utente.
			\item L'utente viene autenticato e la chat viene salvata nel sistema.
		\end{enumerate}
		\item \textbf{Estensioni}:
		\begin{itemize}
			\item Errore: nome utente \glock{Telegram} non riconosciuto (UC 14.3)
			\item Errore: nessuna risposta dal bot \glock{Telegram} (UC 20)
		\end{itemize}
	\end{itemize}
	...
	\begin{itemize}
		\item \textbf{Attori Primari}: Utente non autenticato.
		\item \textbf{Descrizione}: L'autenticazione della chat tra il bot e l'utente non va a buon fine dal momento che il nome utente associato all'account di \glock{Telegram} non è censito nel sistema.
		...
		\item \textbf{Scenario Principale}:
		\begin{enumerate}
			\item Il bot \glock{Telegram} riceve in input il comando che viene inoltrato al sistema, insieme alle informazioni relative alla chat e all'autore del messaggio.
