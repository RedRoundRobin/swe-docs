        ...
        \subsubsection{Strumenti}
            ...
            \paragraph{Programmi per il controllo ortografico}
                ...
                \subparagraph{Text Studio}
                	Sono stati utilizzati i correttori ortografici integrati nell'\glock{ide} Text Studio per la redazione della documentazione.
            ...
            \paragraph{Analisi statica del codice}
                ...
                Checkstyle è un plugin che viene integrato dal gestore di dipendenze \textit{maven} per eseguire dei controlli di verifica delle regole sintattiche del codice di programmazione prodotto in Java. Permette di essere configurato direttamente da un file nella cartella di progetto, denominato \textit{checkstyle.xml}, con tutte le regole opportune. In particolare:
                \begin{itemize}
                    \item a ogni build viene eseguita l'analisi statica e vengono segnalati i warning tramite errori;
                    \item il file \textit{checkstyle.xml} deve essere versionato.
                \end{itemize}

                \subparagraph{Prettier} % TODO spostare in p. di sviluppo, da usare prima del Push tramite npm, integrato con package.json
                ...
                Prettier è uno strumento per il controllo di conformità del codice, che viene usato per il linguaggio Javascript. Analogo a \textit{checkstyle}, permette di eseguire i medesimi controlli sintattici con delle regole definite. 


                \subparagraph{SonarLint} % TODO da spostare in strumenti di sviluppo, si allinea solo a SonarCloud.

                \begin{itemize}
                    \item \textbf{Componenti software:} swe-api, swe-kafka-db, swe-gateway, swe-swe-telegram, swe-webapp 
                    \item \textbf{Linguaggio:} tutti % TODO da ricontrollare
                \end{itemize}

                SonarLint è un plugin della suite di SonarCloud, che permette di integrare un \textit{linter} direttamente nell'IDE in cui si scrive codice di programmazione. Nel nostro caso, è compatibile con tutta la suite di JetBrains (IntelliJ Idea, PHPStorm) e permette di fare uso dei plugin integrati in precedenza per determinare problemi nelle regole di analisi statica.
                

                \subparagraph{ESLint e Prettier} 
